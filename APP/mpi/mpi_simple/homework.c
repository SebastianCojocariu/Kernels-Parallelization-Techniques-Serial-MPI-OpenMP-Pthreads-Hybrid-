...
void applyFilter(char *filter,image *img_before, image *img_after, int l1, int l2){
...
int main(int argc, char * argv[]) {
	...
	if(rank == 0){
		...
		//aplica filtrele	
			for(i = 0;i < no_filters - 1; i++){
				applyFilter(argv[i + 3], &input, &copy, 1, input.heigth - 1);
		...
		applyFilter(argv[no_filters + 2], &input, &copy, 1, input.heigth - 1);
		//------------------------
