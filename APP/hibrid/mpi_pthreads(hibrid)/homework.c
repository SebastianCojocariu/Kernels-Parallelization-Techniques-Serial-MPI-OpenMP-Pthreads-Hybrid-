...
void *applyFilter(void *args){
	argumente *arg = (argumente*)args;
	char *filter = arg->filter;
	image *img_before = arg->img_before;
	image *img_after = arg->img_after;
	int l1 = arg->start;
	int l2 = arg->end;
	...
	if(img_before->type == '5'){
		...
		//void **aux = (void**)img_before->matrix;
...
void parallel_apply_filter(char *filter, image *before, image *after, int no_lines){
	...
	for(int i = 0;i < num_threads; i++){
		v[i].index = i;
		v[i].img_before = before;
		v[i].img_after = after;
		v[i].start = (i == 0)?1:i * (no_lines/num_threads);
		v[i].end = (i == num_threads - 1)? no_lines - 1: (i + 1)* (no_lines/num_threads) - 1;
		v[i].filter = filter;
	}
	...
	for(int i = 0; i < num_threads; i++) {
		pthread_create(&(tid[i]), NULL, applyFilter,&(v[i]));
	}
...
int main(int argc, char * argv[]) {
	...
	if(rank == 0){
		...
		//aplica filtrele	
			for(i = 0;i < no_filters - 1; i++){
				parallel_apply_filter(argv[i + 3], &input, &copy, input.heigth);
				if (num_workers > 1){
					if(rank == 1){
		...
		parallel_apply_filter(argv[no_filters + 2], &input, &copy, input.heigth);
		//------------------------
